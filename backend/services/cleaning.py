...
def full_etl(dataset_id: str):
    ...
    raw_quality = max(0.0, 100 - (raw_metrics["missing_total"] / max(1, raw_metrics["rows"]) * 100))
    clean_quality = max(0.0, 100 - (clean_metrics["missing_total"] / max(1, clean_metrics["rows"]) * 100))
