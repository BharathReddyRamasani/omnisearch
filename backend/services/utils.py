...
def datasetdir(dataset_id: str) -> str:
    ...
    os.makedirs(path, exist_ok=True)
    return path
def load_ingested(dataset_id: str) -> pd.DataFrame:
    path = os.path.join(datasetdir(dataset_id), "ingested.csv")
    if not os.path.exists(path):
        raise HTTPException(status_code=404, detail="Ingested dataset not found")
    return pd.read_csv(path)
